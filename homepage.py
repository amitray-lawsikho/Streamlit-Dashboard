...
def get_live_stats():
    try:
        ...
        """).to_dataframe()
        call_time = call_df["updated_at_ampm"].iloc[0] if not call_df.empty else "N/A"
        ...
        try:
            ...
            """).to_dataframe()
            rev_time = rev_df["updated_at_ampm"].iloc[0] if not rev_df.empty else "N/A"
...
# ─────────────────────────────────────────────
# CSS  — plain string, no f-prefix, no {} conflicts
# ─────────────────────────────────────────────
st.markdown("""
<style>
...
.hub-title {
    ...
    background: linear-gradient(135deg, #FFFFFF 0%, #F97316 55%, #EF4444 100%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;
}
.hub-subtitle {
    font-size: .85rem; color: rgba(255,255,255,.35);
    font-family: 'DM Mono', monospace; letter-spacing: .5px; margin-bottom: 1.8rem;
}
.hub-divider {
...
.stats-row {
    ...
    flex-wrap: wrap; padding: 0 2rem; margin-bottom: 3rem;
    position: relative; z-index: 1;
}
.stat-card {
...
.stat-card:hover { border-color: rgba(249,115,22,.25); background: rgba(249,115,22,.04); }
.stat-icon {
...
.stat-label {
    ...
    letter-spacing: .8px; color: rgba(255,255,255,.35); margin-bottom: .2rem;
}
.stat-val { font-size: .85rem; font-weight: 600; color: #fff; font-family: 'DM Mono', monospace; }
.live-pill {
...
.app-card {
    ...
    overflow: hidden;
    transition: transform .22s cubic-bezier(.4,0,.2,1), border-color .22s, box-shadow .22s, background .22s;
    backdrop-filter: blur(12px);
}
.app-card:hover { transform: translateY(-4px); text-decoration: none !important; }
...
.card-bg-glow {
    ...
    filter: blur(60px); opacity: .12; pointer-events: none; transition: opacity .3s;
}
...
.card-title { font-size: 1.05rem; font-weight: 700; color: #FFF; letter-spacing: .2px; }
.card-desc  { font-size: .78rem; color: rgba(255,255,255,.45); line-height: 1.6; flex: 1; }
.card-chips { display: flex; flex-wrap: wrap; gap: .4rem; margin-top: .3rem; }
.chip {
    font-size: .62rem; font-weight: 600; color: rgba(255,255,255,.5);
    background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.1);
    border-radius: 20px; padding: 2px 9px; letter-spacing: .3px;
}
.card-link {
    font-size: .75rem; font-weight: 700; color: rgba(255,255,255,.5);
    margin-top: .4rem; display: flex; align-items: center; gap: .3rem; transition: color .2s;
}
...
# ─────────────────────────────────────────────
# HTML RENDER — f-string for live BQ values
# ─────────────────────────────────────────────
st.markdown(f"""
<div class="hub-wrapper">
  ...
  <div class="stats-row">
    <div class="stat-card">
      ...
      <div class="stat-body">
        ...
        <div class="stat-val">{call_time}</div>
        <div class="stat-val" style="font-size:.68rem;color:rgba(255,255,255,.28);margin-top:.1rem;">{call_cnt} records</div>
      </div>
    ...
    <div class="stat-card">
      ...
      <div class="stat-body">
        ...
        <div class="stat-val">{rev_time}</div>
        <div class="stat-val" style="font-size:.68rem;color:rgba(255,255,255,.28);margin-top:.1rem;">{rev_cnt} records</div>
      </div>
      <span class="live-pill">● Live</span>
    </div>
    <div class="stat-card">
  ...
  <div class="cards-grid">
    <a class="app-card card-call" href="https://dashboard-lawsikho-call.streamlit.app/" target="_blank">
      <div class="card-bg-glow"></div>
      <div class="card-icon">🔔</div>
      <div class="card-title">Calling Metrics</div>
      <div class="card-desc">Full CDR analysis across Ozonetel, Acefone &amp; Manual calls. Agent-level performance, break tracking, productive hours &amp; team leaderboards.</div>
      <div class="card-chips">
        <span class="chip">Ozonetel</span><span class="chip">Acefone</span>
        <span class="chip">Manual</span><span class="chip">Live</span>
      </div>
      <span class="card-link">Open Dashboard <span>→</span></span>
    </a>
    <a class="app-card card-rev" href="https://dashboard-lawsikho-revenue.streamlit.app/" target="_blank">
      <div class="card-bg-glow"></div>
      <div class="card-icon">💰</div>
      <div class="card-title">Revenue Metrics</div>
      <div class="card-desc">Enrollment revenue, target achievement &amp; caller-level breakdown. Course performance, source mix &amp; team leaderboards.</div>
      <div class="card-chips">
        <span class="chip">Enrollments</span><span class="chip">Targets</span>
        <span class="chip">Courses</span><span class="chip">Live</span>
      </div>
      <span class="card-link">Open Dashboard <span>→</span></span>
    </a>
    <a class="app-card card-leads" href="#" style="pointer-events:none;cursor:default;">
      <div class="card-bg-glow"></div>
      <div class="card-icon">📊</div>
      <div class="wip-badge">🚧 Work in Progress</div>
      <div class="card-title">Lead Metrics</div>
      <div class="card-desc">Pipeline health, lead source quality, conversion rates &amp; funnel drop-off analysis. Coming soon — currently in development.</div>
      <div class="card-chips">
        <span class="chip">Pipeline</span><span class="chip">Funnel</span>
        <span class="chip">Conversion</span><span class="chip">Soon</span>
      </div>
