...
# --- 3. Data Fetching Functions (CACHING REMOVED TO PREVENT STALE DATA) ---
def get_metadata():
...
def fetch_call_data(start_date, end_date):
    ...
    if not df_ozo.empty:
        ...
        df_ozo['direction'] = df_ozo['direction'].str.lower().replace({'manual': 'outbound', 'inbound': 'inbound'})
    ...
    with st.spinner('Calculating metrics...'):
        ...
        if df_raw.empty:
            ...
            if df.empty:
                ...
                # FIX: Include Ozonetel callers even if duration is 0 (Unanswered calls)
                ...
                if df_filtered.empty:
                    ...
                    m2.metric("Calls by Source", f"A: {len(df_filtered[df_filtered['source'] == 'Acefone'])} | O: {len(df_filtered[df_filtered['source'] == 'Ozonetel'])}")
                    ans_total = len(df_filtered[df_filtered['status'].str.lower() == 'answered'])
                    m3.metric("Pick Up Ratio %", f"{(ans_total / df_filtered['call_id'].nunique() * 100):.1f}%")
                    m4.metric("Active Callers", len(report_df))
                    m5.metric("Avg Productive Hrs", format_dur_hm(report_df["raw_prod"].mean()))
                    st.divider()
                    ...
                    st.dataframe(final_df.style.apply(style_total_row, axis=1).set_properties(**{'white-space': 'pre-wrap'}), column_order=["IN/OUT TIME", "CALLER", "TEAM", "TOTAL CALLS", "CALL STATUS", "PICK UP RATIO %", "CALLS > 3 MINS", "CALLS 15-20 MINS", "20+ MIN CALLS", "CALL DURATION > 3 MINS", "PRODUCTIVE HOURS", "BREAKS (>=15 MINS)", "REMARKS"], use_container_width=True, hide_index=True)
                    st.divider()
                    ...
                    if not cdr_csv.empty:
                        ...
                        cdr_csv = cdr_csv[[c for c in t_cols.keys() if c in cdr_csv.columns]].rename(columns=t_cols)
                        if 'Call DateTime' in cdr_csv.columns: cdr_csv['Call DateTime'] = cdr_csv['Call DateTime'].dt.strftime('%Y-%m-%d %H:%M:%S')
